...
courierClient.subscribe(("client/%c/receive", QoS.one))
```
...
A placeholder value can also be split by a delimiter to use a single part of it in the topic. The syntax is `(<placeholder>,<delimiter>,<index>)` where `<index>` is **0-based**. For example, if the client id is `region:tenant:device`, then `(%c,:,2)` splits it by `:` into `[region, tenant, device]` and picks the part at index `2`, so `user/(%c,:,2)/send` resolves to `user/device/send`. An index that is out of range leaves the topic unresolved and logs an error.

Placeholders can be combined freely in the same topic, e.g. `user/(%c,:,0)/%u`.
